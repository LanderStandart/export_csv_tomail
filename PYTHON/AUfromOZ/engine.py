...
class Db:
    def __init__(self,host,database):
        self.conn = fdb.connect(host=host, database=database, user='sysdba', password='masterkey')
...
class ExportData(Db):
 ...
 #Импортируем соответсвующую библиотеку для выбранной выгрузки
    ...
    def planner(self):
        ...
        last_start = read_ini(self.firm.upper(), 'LAST_START', self.firm.lower())
        period = read_ini(self.firm.upper(), 'PERIOD', self.firm.lower())
        period_hour =read_ini(self.firm.upper(), 'PERIOD_HOUR', self.firm.lower())
        if not period:
        ...
        if (period == 'MONTH' and last_start_diff.days < 31) or (period == 'DAY' and last_start_diff.days < 1) \
            ...
            res = 1
            read_ini(self.firm.upper(), 'LAST_START', self.firm.upper(), save=1, values=now.strftime('%d.%m.%Y %H:%M:%S'))
...
def get_profile(firm):
    section = 'BASE_CONF'
    profiles_on = read_ini(firm, 'PROFILES_ON', firm) if read_ini(firm, 'PROFILES_ON', firm) else read_ini(section, 'PROFILES_ON')
    profiles_off = read_ini(firm, 'PROFILES_OFF', firm) if read_ini(firm, 'PROFILES_OFF', firm) else read_ini(section, 'PROFILES_OFF')
    if profiles_on:
