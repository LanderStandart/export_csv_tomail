...
class Sozvezdie(Db):
    def __init__(self, profile_id=None):
        ...
        self.dep_code = read_ini(self.conf, 'DEP_CODE', self.path_ini) if int(self.alone) == 1 else profile_id
    ...
    def prepare_sql(self, sql, pid, da_strt, da_end):
        sql1 = sql.replace(':pid', str(pid))
        sql1 = sql1.replace(':date_start', "'" + da_strt.strftime('%d.%m.%Y:00:00:00') + "'")

        sql1 = sql1.replace(':datestart1', "'" + (da_strt + datetime.timedelta(days=1)).strftime(
            '%d.%m.%Y:00:00:00') + "'") if self.type else sql1.replace(':datestart1', "'" + da_strt.strftime(
            '%d.%m.%Y:00:00:00') + "'")
        ...
        if self.alone != '1':
            ...
            sql1 = sql1.replace(
                ...
                f"(select caption from g$profiles g where g.id={str(self.profile_id)})as map_pharmacy_name,")

        return sql1

    def create_export(self, root, element, data, head_file, sql,  gl_root=None):
        # Формируем шапку XML файла
        if self.type:
            attrib = {"type": root, "map_pharmacy_ids": self.dep_code}
        else:
            attrib = {"type": root, "datestart": self.date_start.strftime("%Y-%m-%dT%H:%M:%S"),
                      "dateend": self.date_end.strftime("%Y-%m-%dT23:59:59"), "map_pharmacy_ids": self.dep_code}
        ...
        for pid in data:
            ...
            sql2 = self.prepare_sql(sql1, pid, da_strt, da_end)
            ...
            for row in p:#обрабатываем результаты запроса

                el1 = XML().add_element(element, self.el)

                if 'move_first' in sql:
                    #                   print (row[2], row[21])
                    self.doc_dates1.append([row[2],
                                            datetime.datetime.strptime(read_ini(self.conf, 'DATE_START', self.path_ini),
                                                                       "%d.%m.%Y")])
                    ...
                    if self.type and row[21] == datetime.datetime.strptime(
                            read_ini(self.conf, 'DATE_START', self.path_ini), "%d.%m.%Y"):
    ...
    def get_Data(self):
        ...
        if self.type:
            self.date_start = self.date_start - datetime.timedelta(days=1)
        gl_root = XML().add_root(root='map-actions',
                                 attrib={"client_id": read_ini(self.conf, 'CLIENT_ID', self.path_ini)})
        XML().add_element(element='data_version', root=gl_root, data='6')

        self.create_export(gl_root=gl_root, root='batches', element='batch', data=parts, head_file='goods',
                           sql='action_batch')
        logger.info('distr')
        self.doc_dates = []
        self.doc_dates1 = []
        ...
        if self.type:
            ...
            self.create_export(gl_root=gl_root, root='distributions', element='distribution', data=parts,
                               head_file='move', sql='action_move_first')
            self.create_export(root='distributions', element='distribution', data=part_id, head_file='move',
                               sql='action_move')
        else:
            self.create_export(gl_root=gl_root, root='distributions', element='distribution', data=part_id,
                               head_file='move', sql='action_move')
        ...
        FTP_work(self.conf).upload_FTP('./' + fn1[0] + '.tmp',
