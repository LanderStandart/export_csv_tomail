    ...
    def upload_FTP(self, file_name, isbynary=None, extpath=None, rename=None):
        ...
        if self.status == '1':
            ...
            try:
                ...
                logger.info(f'Connect to FTP - {self.host}:{self.port} for Upload {self.file_name}')
                ...
                try:
                    ftp = FTP()
                    ftp.encoding = 'utf-8'
                    logger.info(f'Connect to FTP- {self.host}:{self.port}')
                    ftp.connect(self.host, self.port,timeout=10)
                    ftp.login(self.ftp_user, self.ftp_password)
                except :
                    logger.error(f'Нет Связи с ФТП файл {self.file_name} не выгружен  ')
                    return
    ...
    def sinhro(self,REMOTE_FOLDER,LOCAL_FOLDER):
        if self.status == '1':
            try:
                ...
                logger.info(f'{local_file}-Загружен OK!')
