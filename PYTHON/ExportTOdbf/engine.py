...
# from system import System.read_ini, list_file_in_path, get_File, clear_export_path,existPath,put_file
...
class Db(System):
    def __init__(self):
        logger = System.get_logger(self, __name__)
        section = 'BASE_CONF'
...
class ExportData(Db):
 ...
 #Импортируем соответсвующую библиотеку для выбранной выгрузки
        ...
        if int(System.System.read_ini('PARAMS', self.firm.lower())):
            try:
                entity=System.read_ini(self.firm.upper(), 'ENTITY', self.firm.lower())
                if int(System.read_ini('BASE_CONF', 'ALONE')):
                    firmname().get_Data()
                elif not entity and not int(System.read_ini('BASE_CONF', 'ALONE')):

                    SQL_profile = f"SELECT id,caption FROM G$PROFILES where id {get_profile(self.firm.upper())} and status=0 and dbsecurekey is not null and dbsecurekey not containing('!') "
    ...
    def planner(self):
        ...
        last_start = System.read_ini(self.firm.upper(), 'LAST_START', self.firm.lower())
        period = System.read_ini(self.firm.upper(), 'PERIOD', self.firm.lower())
        period_hour =System.read_ini(self.firm.upper(), 'PERIOD_HOUR', self.firm.lower())
        if not period:
            logger.info('не знаю периуд запускаемся')
            res=1
            return res
        last_start_diff = now-datetime.datetime.strptime(last_start,'%d.%m.%Y %H:%M:%S')
        logger.info(f'Последний раз запускали {last_start} периуд запуска {period} прошло {last_start_diff}')
        if (period == 'MONTH' and last_start_diff.days < 31) or (period == 'DAY' and last_start_diff.days < 1) \
            ...
            logger.info('Следующий запуск еще не подошел')
            res = 0

            return res
        else:
            res = 1
            System.read_ini(self.firm.upper(), 'LAST_START', self.firm.upper(), save=1, values=now.strftime('%d.%m.%Y %H:%M:%S'))
...
def get_profile(firm):
    section = 'BASE_CONF'
    profiles_on = System.read_ini(firm, 'PROFILES_ON', firm) if System.read_ini(firm, 'PROFILES_ON', firm) else System.read_ini(section, 'PROFILES_ON')
    profiles_off = System.read_ini(firm, 'PROFILES_OFF', firm) if System.read_ini(firm, 'PROFILES_OFF', firm) else System.read_ini(section, 'PROFILES_OFF')
    if profiles_on:
...
def clear_string(text,code='cp866'):
        text = re.sub("^\s+|\n|\r|\s+$", '', text)
