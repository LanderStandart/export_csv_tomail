...
//(с) 10.02.2020 Дмитрий Дрыняев Стандарт-Н
// для клиента
//-------------------------------------------------------
...
const
 path=extractfiledrive(application.ExeName)+'\Standart-N_new\proapteka\'; //путь для выгрузки файлов
...
Begin
Result:='select  d.id||dd.id as IdLotMovement,
           '''' as DepartmentCode,
           ...
           iif(d.doc_type =3,(select s from UTPR_DATETOSTR(d.commitdate,1)),'''') as ChequeDateModified,
          ...
          abs(iif(d.doc_type =3,cast(dd.discount as numeric(9,2)),'''')) as Discount,
...
begin
Result:='select * from PR_WAREBASE_PROAPTEKA';
end;
//----------------------------------------------------------------------------------------------------------------------------
...
Result:='select
           ...
           '''' as DepartmentCode,
           iif(d.doc_type<>20,d.agent_id,''999999'') as IDDepartment,
...
Result:='select
 ...
 iif(docs.agent_id<0,''Ввод остатков'',agents.caption) as Name,
...
begin
Result:='select  d.id||dd.id as IdDocumentItem,
        '''' as IDDiscount,
...
begin
Result:='Select d.id as IdDiscountProgram,
  d.dsc_text as DiscountProgramName,
 abs(d.percent) as DiscountProgramPercent,
 '''' as DiscountType,
 d.barcode as DiscountCardBarcode
 from discounts d';
end;
...
DepartmentCode:='10000'; // Код профиля Аптеки
