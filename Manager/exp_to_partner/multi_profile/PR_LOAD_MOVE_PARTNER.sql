...
if (mindate<:dateend-93) then delete from partner_load_move where doc_commitdate<:dateend-93;-- Удаляет даные относящиеся к предыдущему периоду выгрузки (прошлый квартал)
 ...
 for select
     ...
     group by dd.g$profile_id, dd.part_id, dd.doc_id,dd.discount,dd.sum_dsc,dd.summa_o,dd.summa,dd.price
     into :drug_code, :doc_id, :quant,  :disk_T, :disk_sum, :Sum_Zak, :Sum_Rozn, :Cena_Rozn, :profile_id do
 begin
 INSERT INTO partner_load_move (d_type,docnum,docdate,supplier,supplier_inn,device_num,n_chek,fio_chek,pp_teg,drug_name,izg_id,drug_producer_name,bcodeizg,drug_producer_country,
                                                cena_zak,seria,godendo_date,barcode,drug_code,quant,disk_t,disk_sum,sum_zak,sum_rozn,cena_rozn,doc_commitdate,g$profile_id)
    ...
    select
         ...
         vcountry.svalue as Drug_Producer_country,
          ...
          p.barcode,
          :drug_code,
                      :quant,  :disk_T, :disk_sum, :Sum_Zak, :Sum_Rozn, :Cena_Rozn ,d.docdate, d.g$profile_id


    from docs d
        left join parts p on p.id = :drug_code and p.g$profile_id=:profile_id
        ...
        and d.id=:doc_id and d.g$profile_id=:profile_id and
        (select (select Upper(a1.caption) from agents a1 where a1.id=pr.agent_id  ) from pr_getmotherpart(p.id, d.g$profile_id)pr) containing Upper('Катрен') -- and d.docdate between :DATESTART and :DATEEND
