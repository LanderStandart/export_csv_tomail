...
declare variable PROFILE_ID DM_ID_NULL;
begin
...
insert into partner_load_begin(part_id,quant, q_date,g$profile_id)
  select dd.part_id,cast(abs(sum(dd.quant)) as dm_double),:q_date,dd.g$profile_id from doc_detail dd
  join g$profiles g on g.id = dd.g$profile_id and g.partner_id is not null
  join docs d on dd.doc_id=d.id and d.g$profile_id=dd.g$profile_id
  where dd.doc_commitdate<:q_date  and (select (select Upper(a1.caption) from agents a1 where a1.id=pr.agent_id ) from pr_getmotherpart(dd.part_id, dd.g$profile_id)pr) containing Upper('Катрен')

  group by dd.g$profile_id,dd.part_id having abs(sum(dd.quant))>0.001;
         ...
         for
          ...
          from
             (  select
                 ...
                 and (select (select Upper(a1.caption) from agents a1 where a1.id=pr.agent_id  ) from pr_getmotherpart(dd.part_id, dd.g$profile_id)pr) containing Upper('Катрен')
                 group by dd.g$profile_id,dd.part_id ) q
                group by g$profile_id,part_id
                having sum(q.quant)>0.001
 into  :part_id, :quant, :profile_id  do
         begin
           insert into partner_load (docdate,docagent,docnum, inn,izg_id,sizg,scountry,sname,bcode_izg,price_o,price,godendo_date,seria,barcode,doc_commitdate,part_id,quant,q_date,g$profile_id)
            select
                (select s from utpr_datetostr(:rep_date)) as docdate,
                (select (select a.caption from agents a where a.id=pr.agent_id  ) from pr_getmotherpart(p.id,p.g$profile_id) pr)as docagent,
            ...
            from parts p
                ...
                where p.id = :part_id and p.g$profile_id= :profile_id ;
