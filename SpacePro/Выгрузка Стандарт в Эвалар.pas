...
begin
     ...
     result:=extractfiledrive(application.ExeName)+path+copy(StringReplace(date,'.','-',1),4,7)+'-'+FileName;
...
order by dd.doc_commitdate';
  CreateSheets('SALE',SQL,'Дата продажи','Кассир','Название товара','Название Аптеки','Город',
...
where dd.doc_commitdate between '''+Date_start+''' and '''+date_end+''' and dd.g$profile_id in (8,9,12)
order by dd.doc_commitdate'
  CreateSheets('SUPP',SQL,'Дата закупки','Дистрибутор','Название товара','Название Аптеки','Город',
...
SQL:='select first 10
    ...
    (select substring(g.caption from position('' '' in g.caption) for position(''тел'' in g.caption)-position('' '' in g.caption)) from g$profiles g where g.id=dd.g$profile_id) as adress,
     sum(-dd.summa_o) as summa
from doc_detail dd
 inner join docs d on d.id=dd.doc_id and d.g$profile_id=dd.g$profile_id and d.doc_type = 3
 where dd.doc_commitdate between '''+Date_start+''' and '''+date_end+''' and dd.g$profile_id in (8,9,12)
 group by dd.g$profile_id';

  CreateSheets('TSALE',SQL,'Название Аптеки','Город','Адрес аптеки','Сумма','','','','','',
              'Profile','City','adress','Summa','','','','',''  );

//выгружаем на сервер Эвалар
  t:='-F -u standartm -p 314728726 ftp://af.evalar.ru \ C:\Standart-n\Эвалар\*.xlsx';
