...
return $this->MyCURL($url,'PUT',$autriz,'',0);
...
$response=$this->MyCURL($base_url,'POST',$autriz,$Data,1);
 return $response;
}
...
function CompilePacket($datesale,$profile=null)
{
  ...
  while ($row=ibase_fetch_row($result))
{
            ...
            if ($count_pac == $maxpacket) {print_r('<bt>'.$count_pac.'<br>');
              $res = substr($res,0,-1);
              $res = $res.']';
...
if (isset($profile)){
  ...
  {$sql="SELECT  * from PHARMIT_OUT_DECADE";}

$datestart = DateTime::createFromFormat('Ymd');
...
if (isset($month))
    {$prefix = 'MonthSale:';
     $datesale = $prefix.$datestart->format('Ym');}
