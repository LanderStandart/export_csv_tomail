...
function getDetail($trn, $ware_id, &$izg, &$country, &$i, &$profiles_count, &$min_price, &$max_price){
  ...
  coalesce(scountry,'')
from warebase_g wb
where quant>0 and name_id='".$ware_id."'
...
//print_r($sql);
  ...
  while ($row=ibase_fetch_row($q)){
    $s="";
    if ($row[13]!=""){$s=$row[13];}
	if ($row[12]!=""){if ($s!=""){$s=$row[12].", ".$s;};}
    $i++;
    $rtn.="<tr>
    <td class=\"t_profiles_td_odd\">$i) ".$row[2]."</td>
    <td>".$s."</td>
    <td class=\"t_profiles_td_odd\">".$row[3]."</td>
    <td class=\"t_profiles_td_quant\">".$row[4]."</td>
    <td class=\"t_profiles_td_odd\">".$row[5]."</td>
    <td>".$row[6]."</td>
    <td class=\"t_profiles_td_price\">".$row[7]."</td>
    <td>".$row[8]."</td>
    <td class=\"t_profiles_td_odd\">".$row[9]."</td>
    <td>".$row[10]."</td>
	<td class=\"t_profiles_td_odd\">".$row[11]."</td>
  </tr>";
...
//	  echo "[".$query."]";
	...
	$sql="select distinct name_id, sname from wares ".$where." order by sname, name_id";
//	echo $sql;
//	return;
    if ($num==0){
	  $q=ibase_query($trn,$sql) or die("error: query ".ibase_errmsg()." [".$sql."]");
...
//	echo "<br><br>Всего: ".($i+$_SESSION["LAST_NUM"]);
  }
  init();
//  echo '<div>112233</div>';
/*
